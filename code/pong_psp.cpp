...
unsigned int rgb;		//used for converting from rgb to unsigned int
...
void draw_walls(void) {
  	...
  	setColor(255,0,0);
  	printf("psps walls should be red\n");
...
void setColor(float R, float G, float B){
	 unsigned int hexValue = (unsigned int) ((r & 0xff) << 16) + ((g & 0xff) << 8) + (b & 0xff);
	 sceGuColor(hexValue);
}
#endif
